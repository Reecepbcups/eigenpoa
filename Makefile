...
HTTPS_GIT := github.com/reecepbcups/eigenpoa.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=eigenpoa \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=eigned \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/eigned ./cmd/eigned
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/eigned.exe ./cmd/eigned
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/eigned
...
draw-deps:
	...
	@goviz -i ./cmd/eigned -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c eigenpoa --local -f chains.yaml
endif
...
set-testnet-configs:
	eigned config set client chain-id localchain-1
	eigned config set client keyring-backend test
	eigned config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | eigned keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | eigned keys add acc1 --recover`
