...
# then log output of file /code/bin/eigned
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/eigned \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/eigned | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/eigned /usr/bin/eigned
